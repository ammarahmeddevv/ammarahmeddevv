...
function projectBlock(p) {
  ...
  out.push(p.stack.map(chip).join(" "));
  out.push("");
  const links = linkRow([["Live", p.live], ["Source", p.source]]);
  if (links) out.push(links);
  else if (p.access) out.push(`<sub>${p.access}</sub>`);
  out.push("");
  return out.join("\n");
}
...
function readme() {
  ...
  if (also.length) {
    ...
    for (const p of also) {
      ...
      o.push(`${p.stack.map(chip).join(" ")}${links ? "  ·  " + links : ""}`);
      o.push("");
    }
  }
  o.push("---");
  ...
  o.push("| | |");
  o.push("|---|---|");
  for (const g of P.stack.groups) o.push(`| **${g.label}** | ${g.items.map(chip).join(" ")} |`);
  o.push("");
  o.push("---");
  o.push("");
